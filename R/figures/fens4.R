...
fitModelForAll <- function(data, allData) {
  dataAverage <- data %>%
  ...
  modelResult <- foreach(pair = typePairs, .combine = rbind) %do% {
    ...
    if (all(pair == typePairs[ 1 ] %>% unlist())) {
      fit_res <- fit_res[ , y.position := allAverage[ , max(variable) ] + allAverage[ , max(abs(variable)) ] * 0.06 ]
    }
    ...
    if (all(pair == typePairs[ 3 ] %>% unlist())) {
      fit_res <- fit_res[ , y.position := allAverage[ , max(variable) ] + allAverage[ , max(abs(variable)) ] * 0.12 ]
    }

    if (all(pair == typePairs[ 4 ] %>% unlist())) {
      fit_res <- fit_res[ , y.position := allAverage[ , max(variable) ] + allAverage[ , max(abs(variable)) ] * 0.18 ]
    }
...
vm_colors <- brewer.pal(11, "RdBu")[ c(1, 2, 10, 11) ]
names(vm_colors) <- c("M1 L23", "M1 L5", "S1 L23", "S1 L5")

plot <- list()
for (region in c("M1 L23", "M1 L5", "S1 L23", "S1 L5")) {
  ...
  p1 <- region_average %>%
    ...
    geom_line(aes(x = Time, y = Vm), color = color, linewidth = 0.5) +
  ...
  res <- fitModelForAll(typeAverage, allData = eventDataAverage)

  p2 <- res$dataAverage %>%
    ggpaired(x = "Type", y = "variable", id = "SID",
             line.color = "gray", line.size = 0.2, point.size = 0, color = "white") +
    geom_boxplot(aes(color = Type, fill = Type), outlier.alpha = 0, alpha = 0.25, linewidth = 0.25) +
    ...
    scale_fill_manual(values = c("B" = colors[ 1 ], "P" = colors[ 2 ], "O" = colors[ 3 ], "L" = colors[ 4 ])) +
    theme(panel.grid.minor = element_blank()) +
    expand_limits(y = c(-72, -22))

  p5 <- ggdraw() +
    draw_plot(p1, 0.062, 0.5, 1 - 0.062, 0.5) +
    draw_plot(p2, 0.068, 0, 1 - 0.068, 0.5)

  # Only for the first in list
  if (region == "M1 L23") {
    p5 <- p5 + draw_plot_label(letters[ 1:2 ], x = c(0, 0), y = c(1, 0.5), size = 9)
  }
...
ggsave(final, filename = "fens/figure4.png", height = 4.5, width = 8)

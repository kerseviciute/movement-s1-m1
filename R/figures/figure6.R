saveRDS(snakemake, ".figure6.R.RDS")
# snakemake <- readRDS(".figure6.R.RDS")
...
for (type in c("Late movement", "Movement offset", "Post-movement")) {
...
plots <- list()
for (type in c("Late movement", "Movement offset", "Post-movement", "Baseline")) {
  plots[[ type ]] <- dataSampleAverage %>%
    .[ Type == type ] %>%
